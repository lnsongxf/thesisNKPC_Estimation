...
%   $Revision: 1.1.10.4 $  $Date: 2013/10/09 06:36:52 $
...
while ~done
    ...
    if constflag && ~flags.gradconst || ~flags.grad 
        ...
        if serialFinDiff         % Call serial finite-differences code
          ...
          % parfinitedifferences. They are available to
          % parfinitedifferences function from the local workers. See
          % optimlib/setOptimFcnHandleOnWorkers.m for more information)
            if (~flags.grad && ~flags.gradconst) 
                % No objective gradients, no constraint gradients
                [gf,gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),numEvals] = ...
            ...
            elseif ~flags.gradconst 
                % No constraint gradients; objective gradients supplied
                [gf,gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),numEvals] = ...
                    parfinitedifferences(XOUT,lb,ub,f,nc(nonlIneqs_idx), ...
                    nc(nonlEqs_idx),1:numberOfVariables,OPTIONS,sizes,gf, ...
                    gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),finDiffFlags,[],varargin{:});
            elseif ~flags.grad    % No objective gradients, constraint gradients supplied
                [gf,~,~,numEvals] = parfinitedifferences(XOUT,lb,ub,f,[],[], ...
                    1:numberOfVariables,OPTIONS,sizes,gf,[],[],finDiffFlags,[],varargin{:});
            end
        end 
